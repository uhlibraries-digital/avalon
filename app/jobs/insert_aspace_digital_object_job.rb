...
module InsertAspaceDigitalObjectJob
  class Create < ActiveJob::Base
    ...
    queue_as :insert_aspace_digital_object
    def perform(media_object_id)
      ...
      ids.each.with_index do |id, index|
        if id[:source] == 'aspace uri'
          ids[index][:id] = Settings.archivesspace.public_url + aspace_uri
        end
      end
    ...
    def identifier
      ...
      ids.each do |i|
        if i[:source] == 'digital object'
          ark_id = i[:id].match(/(ark:\/[0-9a-z\/]+)/).to_s
          return Settings.greens.base_uri + ark_id
        end
      end
      return ''
    end
  ...
  class ArchivalObject < Flexirest::Base
    base_url Settings.archivesspace.endpoint
    ...
    def add_authentication_details(name, request)
      @session = InsertAspaceDigitalObjectJob::Session._request(
        Settings.archivesspace.endpoint + '/users/' +
        Settings.archivesspace.username + '/login',
        :post,
